...
const hasCreatedAt = pragmaForm.some((col) => col && typeof col.name === "string" && col.name === "createdAt");
if (!hasCreatedAt) {
  db.exec("ALTER TABLE formations ADD COLUMN createdAt TEXT;");
}
const hasUpdatedAt = pragmaForm.some((col) => col && typeof col.name === "string" && col.name === "updatedAt");
if (!hasUpdatedAt) {
  db.exec("ALTER TABLE formations ADD COLUMN updatedAt TEXT;");
}
try {
  const pragmaForm2 = db.prepare("PRAGMA table_info(formations)").all();
  const hasCreatedAt2 = pragmaForm2.some((col) => col && typeof col.name === "string" && col.name === "createdAt");
  if (!hasCreatedAt2) {
    db.exec("ALTER TABLE formations ADD COLUMN createdAt TEXT;");
  }
  const hasUpdatedAt2 = pragmaForm2.some((col) => col && typeof col.name === "string" && col.name === "updatedAt");
  if (!hasUpdatedAt2) {
    db.exec("ALTER TABLE formations ADD COLUMN updatedAt TEXT;");
  }
} catch (e) {
...
try {
  ...
  const hasOld = Array.isArray(pragma) && pragma.some((col) => col && typeof col.name === "string" && col.name === "utilisateurId");
  if (hasOld) {
    db.exec(`
...
const hasAlt = pragmaGalerie.some((col) => col && typeof col.name === "string" && col.name === "alt");
if (!hasAlt) {
...
const hasNotes = pragmaRes.some((col) => col && typeof col.name === "string" && col.name === "notes");
if (!hasNotes) {
...
`);
const rowSite = db.prepare("SELECT COUNT(*) as count FROM site_identity").get();
if (rowSite.count === 0) {
...
const rowTeam = db.prepare("SELECT COUNT(*) as count FROM team").get();
if (rowTeam.count === 0) {
  const teamSeed = [
    {
      nom: "Neill Dupont",
...
const row = db.prepare("SELECT COUNT(*) as count FROM services").get();
if (row.count === 0) {
  const servicesSeed = [
    {
...
const rowForm = db.prepare("SELECT COUNT(*) as count FROM formations").get();
if (rowForm.count === 0) {
...
const rowGalerie = db.prepare("SELECT COUNT(*) as count FROM galerie").get();
...
const rowAvis = db.prepare("SELECT COUNT(*) as count FROM avis").get();
if (rowAvis.count === 0) {
  const seed = [
    // 2 globaux (accueil)
...
const rowUser = db.prepare("SELECT COUNT(*) as count FROM utilisateurs").get();
if (rowUser.count === 0) {
...
const rowRes = db.prepare("SELECT COUNT(*) as count FROM reservations").get();
if (rowRes.count === 0) {
  const userIds = db.prepare("SELECT id FROM utilisateurs").all().map((u) => u.id);
  const serviceIds = db.prepare("SELECT id FROM services").all().map((s) => s.id);
  const seed = [
...
const rowFact = db.prepare("SELECT COUNT(*) as count FROM factures").get();
if (rowFact.count === 0) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const reservations = db.prepare("SELECT id, userId FROM reservations").all();
...
`);
const rowFaq = db.prepare("SELECT COUNT(*) as count FROM faq").get();
if (rowFaq.count === 0) {
  const seed = [
