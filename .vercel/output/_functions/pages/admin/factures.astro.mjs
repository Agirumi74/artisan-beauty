import { c as createComponent, b as renderTemplate, r as renderComponent, m as maybeRenderHead, e as addAttribute } from '../../chunks/astro/server_BiuKQx4Z.mjs';
import 'kleur/colors';
...
var _a;
const $$Factures = createComponent(async ($$result, $$props, $$slots) => {
  let factures = [];
  try {
      ...
      row.innerHTML = \`
        <form id="edit-fact-form-\${id}" class="flex flex-col md:flex-row gap-2 w-full">
          ...
          <select name="status" class="border rounded px-2 py-1 flex-1" required>
            <option value="paid">Pay\xE9e</option>
            <option value="pending">En attente</option>
            <option value="cancelled">Annul\xE9e</option>
          </select>
...
<\/script>`])), renderComponent($$result, "AdminLayout", $$AdminLayout, {}, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<section class="container mx-auto py-12"> <h1 class="text-3xl font-bold mb-4">Gestion des factures</h1> <form id="add-facture-form" class="mb-8 bg-gray-50 p-4 rounded shadow max-w-2xl mx-auto"> <div class="flex flex-col md:flex-row gap-4 items-end"> <div class="flex-1"> <label for="reservationId" class="block font-semibold">ID Réservation</label> <input id="reservationId" name="reservationId" type="text" required class="w-full border rounded px-3 py-2" placeholder="ID réservation"> </div> <div class="flex-1"> <label for="userId" class="block font-semibold">ID Utilisateur</label> <input id="userId" name="userId" type="text" required class="w-full border rounded px-3 py-2" placeholder="ID utilisateur"> </div> <div class="flex-1"> <label for="amount" class="block font-semibold">Montant</label> <input id="amount" name="amount" type="number" required class="w-full border rounded px-3 py-2" placeholder="Montant (€)"> </div> <div class="flex-1"> <label for="status" class="block font-semibold">Statut</label> <select id="status" name="status" required class="w-full border rounded px-3 py-2"> <option value="paid">Payée</option> <option value="pending">En attente</option> <option value="cancelled">Annulée</option> </select> </div> <div class="flex-1"> <label for="pdfUrl" class="block font-semibold">PDF (URL)</label> <input id="pdfUrl" name="pdfUrl" type="text" class="w-full border rounded px-3 py-2" placeholder="URL PDF (optionnel)"> </div> <button type="submit" class="bg-pink-700 text-white px-4 py-2 rounded hover:bg-pink-800">Ajouter</button> </div> </form> <table class="w-full bg-white rounded shadow mt-8"> <thead> <tr> <th>ID</th> <th>Réservation</th> <th>Utilisateur</th> <th>Montant</th> <th>Statut</th> <th>PDF</th> <th>Créée</th> <th>Actions</th> </tr> </thead> <tbody> ${factures.length === 0 ? renderTemplate`<tr><td colspan="8" class="text-center">Aucune facture</td></tr>` : factures.map((f) => renderTemplate`<tr${addAttribute(`row-fact-${f.id}`, "id")}> <td>${f.id}</td> <td>${f.reservationId}</td> <td>${f.userId}</td> <td>${f.amount} €</td> <td>${f.status}</td> <td>${f.pdfUrl ? renderTemplate`<a${addAttribute(f.pdfUrl, "href")} target="_blank">PDF</a>` : "-"}</td> <td>${f.createdAt?.slice(0, 10)}</td> <td> <button class="bg-blue-600 text-white px-2 py-1 rounded"${addAttribute(`edit-fact-${f.id}`, "id")}>Éditer</button> <button class="bg-red-600 text-white px-2 py-1 rounded ml-2"${addAttribute(`delete-fact-${f.id}`, "id")}>Supprimer</button> </td> </tr>`)} </tbody> </table> </section> ` }));
}, "/home/runner/work/artisan-beauty/artisan-beauty/src/pages/admin/factures.astro", void 0);

const $$file = "/home/runner/work/artisan-beauty/artisan-beauty/src/pages/admin/factures.astro";
const $$url = "/admin/factures";
