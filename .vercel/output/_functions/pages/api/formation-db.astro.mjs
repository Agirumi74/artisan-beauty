...
const GET = async ({ request }) => {
  ...
  const formations = stmt.all(...values).map((f) => ({
    ...f,
    tags: parseArrayField(f.tags),
    steps: parseArrayField(f.steps)
  }));
  const totalRow = db.prepare("SELECT COUNT(*) as count FROM formations" + (filters.length ? " WHERE " + filters.join(" AND ") : "")).get(...values.slice(0, -2));
